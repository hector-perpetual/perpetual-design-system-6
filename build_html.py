...
def circ_icon(x, y, d, fill, hexsize=0.42):
    """Icono circular con hexagono de marca dentro."""
    return (box(x, y, d, d, fill=fill, oval=True, shadow=True)
            + hexagon(x + (d - d * hexsize) / 2, y + (d - d * hexsize) / 2, d * hexsize, WHITE))
...
def s06():  # Vision — fila de 3 iconos circulares
    out = [title(f"Nuestra {AC('vision.')}"),
           txt(0.72, 2.25, 11.0, 0.8,
               ...
               16, DIM, 400, lh=1.4)]
    items = [("Alcance regional", "Presencia en los principales mercados de LatAm.", DATA[0]),
             ("Excelencia operativa", "Procesos eficientes y escalables como ventaja.", DATA[2]),
             ("Innovacion constante", "Adopcion temprana de tecnologia y datos.", DATA[4])]
    for i, (h, b, col) in enumerate(items):
        x = 1.4 + i * 3.95
        out += [circ_icon(x, 3.5, 1.35, col),
...
def s08():  # Approach — fila de iconos circulares con pasos
    ...
    steps = [("01", "Descubrir"), ("02", "Diagnosticar"), ("03", "Diseñar"),
             ("04", "Ejecutar"), ("05", "Medir"), ("06", "Optimizar")]
    d = 1.15
    for i, (n, t) in enumerate(steps):
        x = 0.78 + i * 2.05
        col = DATA[i % len(DATA)]
        out += [circ_icon(x, 3.1, d, col),
